...
function cursorLoop() {
  ...
  const ease = _isHovering ? 0.095 : 0.13;
              ...
              {PROJECTS.map((p, i) => (
                <li key={i}>
                  <svg className="logo" viewBox={p.iconViewBox} fill="none" xmlns="http://www.w3.org/2000/svg">
                    {p.circleColor && <circle cx="512" cy="512" r="512" fill={p.circleColor} />}
                    <path d={p.icon} fill={p.pathColor || "white"} />
                  </svg>
